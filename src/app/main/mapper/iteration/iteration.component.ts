...
export class IterationComponent extends OnDestroyMixin implements OnInit, AfterViewInit, OnDestroy {
  ...
  private isBnode(object): boolean {
    return object.getValueType().getType() === Type.UniqueBnode || object.getValueType().getType() === Type.ValueBnode;
  }
  ...
  private getOnDeleteWarningMessage(triple: Triple): string {
    ...
    const object = triple.getObject();
    if (object) {
      hasChildren = !!(this.hasTypeMappings(object) || this.hasPropertyMappings(object));
    }
    if (hasChildren) {
